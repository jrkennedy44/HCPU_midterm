...
foreach my $key (keys (%set1)) {
	...
	if ($result{$key}->count > 0) {
		my ($min, $max);
		$result{$key}->iterate(sub { $min = $_[0]->min ; $max = $_[0]->max; print $key."\tDifference1\tDifference1\t".$min."\t".$max."\t.\t.\t.\tTarget\n";}, sub {}, );
	}

	$result{$key} = $set2{$key}->difference($set1{$key});
	if ($result{$key}->count > 0) {
		my ($min, $max);
		$result{$key}->iterate(sub { $min = $_[0]->min ; $max = $_[0]->max; print $key."\tDifference2\tDifference2\t".$min."\t".$max."\t.\t.\t.\tTarget\n";}, sub {}, );
	}
}
