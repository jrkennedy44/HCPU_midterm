...
def runCollect( groupid, clean ):
    ...
    if os.path.exists( "%s.set_tmp" % ( groupid ) ):
        os.system( "rm -f %s.set_tmp" % ( groupid ) )

    lSetFiles = glob.glob( "*.Mreps.set" )

    # concatenate all the individual set files
    for setFile in lSetFiles:
        prg = "cat"
        cmd = prg
        cmd += " %s >> %s.set_tmp" % ( setFile, groupid )
        log = os.system( cmd )
        if log != 0:
    ...
    if os.path.exists( "%s.set" % ( groupid ) ):
        os.system( "rm -f %s.set" % ( groupid ) )

    # adapt the path IDs
    if os.path.exists( os.environ["REPET_PATH"] + "/bin/setnum2id" ):
        prg = os.environ["REPET_PATH"] + "/bin/setnum2id"
        cmd = prg
        cmd += " -i %s.set_tmp" % ( groupid )
        cmd += " -o %s.set" % ( groupid )
        cmd += " -v %i" % ( verbose - 1 )
    else:
        prg = os.environ["REPET_PATH"] + "/bin/setnum2id.py"
        cmd = prg
        cmd += " -i %s.set_tmp" % ( groupid )
        cmd += " > %s.set" % ( groupid )
    log = os.system( cmd )
...
def main():
    ...
    # launch Mreps in parallel on all the fasta files in 'query_dir'
    launcher = Launcher.MrepsLauncher( jobdb, query_dir, paramMreps, cdir, tmpdir, job_table, queue, groupid )
    launcher.run()
